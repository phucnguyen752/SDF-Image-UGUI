...
Run these commands from a clean committed checkout in PowerShell after verification. When the working project contains unrelated edits, commit only the reviewed release files and create a detached worktree at that commit for the split. For later releases, replace every `0.5.0` below with the new version.
...
git tag -a 0.5.0 $packageCommit -m "SDF Image 0.5.0"
git push --atomic origin main upm refs/tags/0.5.0
git ls-remote origin refs/heads/main refs/heads/upm refs/tags/0.5.0 'refs/tags/0.5.0^{}'
```

The remote `upm` hash and peeled tag hash (`0.5.0^{}`) must equal `$packageCommit`. Never move an existing release tag; use a new version for changes after publication.

Finally, install `https://github.com/phucnguyen752/sdf-image.git#0.5.0` in a clean Unity 6 project, confirm package identity and sample import, and create the GitHub release from the annotated tag using the verified release notes. Attach a package archive built from that same commit and verify its uploaded digest.
